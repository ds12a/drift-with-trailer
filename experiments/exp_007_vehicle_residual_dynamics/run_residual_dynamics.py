...
from src.dynamics.trailer.residual_dynamics import (
    gen_util_funs,
    D_STATE_DIM,
    D_U_DIM,
    D_EXTRA_DIM,
...
loader = DataLoader.load(
    Path("./experiments/exp_007_vehicle_residual_dynamics/data_proc1.npz"), spec
)
...
nnx.update(
    model, ckpt.restore(Path.cwd() / "src/learning/models/trained/trailer-nokin-best", state)
)
...
try:
    ...
    for _ in range(HISTORY):
        ...
        curr = jnp.concatenate(
            [
                jnp.array([*astuple(state)[:6], env.unwrapped.track.mu]),
                jnp.zeros(2),
                jnp.array([arclen]),
            ]
        )
    ...
    while True:
        ...
        curr = jnp.concatenate(
            [main_slice, jnp.zeros(2), jnp.array([arclen])]
        )  # Control is filled out inside
