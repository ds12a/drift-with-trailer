"""Fiala tractor-trailer dynamics with independently varying surface friction.

State layout:
    [x, y, phi_1, phi_2, v_1x, v_1y, phi_1_dot, phi_2_dot,
     mu_tractor, mu_trailer, arc_len]

The two friction states have zero dynamics.  A caller can therefore replace
them from a simulator/material query between controller steps without changing
the continuous vehicle state.
"""
...
def gen_util_funs(
    ...
    def dynamics(state, u):
        ...
        mu_tractor = jnp.maximum(mu_tractor, 1e-3)
        mu_trailer = jnp.maximum(mu_trailer, 1e-3)
        ...
        fxr = mu_tractor * fzr * jnp.tanh(
            vehicle.mass * commanded / (fzr * mu_tractor)
        )
    ...
    def cost(x, u, t):
        ...
        def combined_traction_penalty(state, action):
            ...
            return pen_f**2 + pen_r**2 + pen_t**2
        ...
        index = jnp.searchsorted(track._cumulative, x[10], side="right") - 1
